...
export function mountDuel(root, duel, { onEnd, ante, speed = 420, portraits = null, tutorial = false, localIdx = 0, input = null, allowMulligan = true, autoStart = true }) {
  ...
  function controls() {
    ...
    switch (req.kind) {
      ...
      case 'piles': return `<div class="hint">${esc(req.text)}</div><div class="piles">${req.piles.map((pile, i) => `<div class="pile"><div class="pile-head">Pile ${i + 1}</div><div class="pile-cards">${pile.cards.length ? pile.cards.map(c => `<span class="pile-card" data-preview="${esc(c.name)}">${esc(c.name)}</span>`).join('') : '<span class="pile-empty">(nothing)</span>'}</div><button class="btn primary" data-pile="${i}">Take pile ${i + 1}</button></div>`).join('')}</div>`;
  ...
  root.addEventListener('click', ev => {
    ...
    if (btn.dataset.pile !== undefined) { act.answer(Number(btn.dataset.pile)); run(); return; }
