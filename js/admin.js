...
function detail(g) {
  ...
  return `<div class="box adm-detail">
    ...
    ${(g.flags || []).length ? `<h4>⚑ Flagged moments</h4>${g.flags.map(f => `<div class="adm-flag"><b>Turn ${f.turn}, ${esc(f.step)}:</b> ${esc(f.note)}<div class="small adm-recent">${(f.recent || []).map(esc).join('<br>')}</div>${f.stack?.length ? `<div class="small">Stack: ${f.stack.map(esc).join(' > ')}</div>` : ''}${board(f.board)}</div>`).join('')}` : ''}
