...
Every duel is recorded — each line of the engine's narration with its turn and step, every action and answer, the prompts shown, script errors, and any moment you flag with the **⚑ Report** button in a duel (that saves the whole board and stack with your note). With `node server.js` or `node relay.js` running, records are appended to `logs/games.jsonl` (kept out of git); the title screen shows how many games this browser holds and can export them as one `.jsonl` file. Every build reports finished games to the collector named in `content/config.json` (`logEndpoint`, the hosted relay by default; records that can't be sent wait in the browser and go on the next visit). Pull them to your machine with `node tools/gamelogs.mjs --pull https://your-relay.onrender.com --token <LOG_TOKEN>` (the token is set on the Render service; on a paid plan the blueprint mounts a 1 GB disk at `/data` so the log survives deploys). Browse every match in the UI at `admin.html` (linked from the title screen): enter the relay's `LOG_TOKEN` to list all collected games, open one for its flagged moments with the board as it stood, errors, decks and the full turn-by-turn log, and filter to flagged or errored matches. Summarise what broke across all games with `node tools/gamelogs.mjs` (add `--flags`, `--errors`, or `--game <id>` to replay one). This is the data the rules engine gets fixed from — flag anything that doesn't do what the card says.
