...
import { makeLogStore } from './logstore.js';

const ROOT = path.dirname(fileURLToPath(import.meta.url));
const logStore = makeLogStore(ROOT);
...
const server = http.createServer((req, res) => {
  ...
  if (logStore.handle(req, res, url)) return;   // game-log collector: POST api/log, GET api/logs (LOG_TOKEN), GET api/log/status
  if (p === '/api/collection') {
...
server.listen(PORT, () => {
  ...
  console.log(logStore.startupNote());
});
