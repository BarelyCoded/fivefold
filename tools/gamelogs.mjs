...
if (flags.length) { console.log('\n== flagged moments =='); for (const { g, f } of flags.slice(0, flag('--flags') ? 1000 : 12)) { console.log(`- [${g.id}] ${g.mode} vs ${g.meta?.opponent || g.players?.[1]?.name} · turn ${f.turn} ${f.step}: ${f.note}`); for (const l of (f.recent || []).slice(-5)) console.log(`      ${l}`); if (f.stack?.length) console.log(`      stack: ${f.stack.join(' > ')}`); } }
