// Headless balance simulator: compiles every enemy deck and plays AI vs AI matches.
//   node tools/simulate.mjs            -> support report + round robin, 20 games per pairing
//   node tools/simulate.mjs 50         -> 50 games per pairing
// Card data is cached in tools/.cardcache.json so repeated runs are offline.
...
// Shim localStorage for the browser modules.
...
const { aiMain1, aiMain2, chooseBlocks } = await import('../js/ai.js');
...
function expand(deck) { const out = []; for (const [n, c] of Object.entries(deck)) for (let i = 0; i < c; i++) out.push(defs.get(n)); return out.filter(Boolean); }

// ---- play a game with two AIs ------------------------------------------------------
async function play(a, b) {
  const duel = new Duel({ player: { name: a.name, deck: expand(a.deck), life: a.life, ai: true }, ai: { name: b.name, deck: expand(b.deck), life: b.life, ai: true } });
  duel.start();
  let guard = 0;
  while (duel.winner === null && guard++ < 400) {
    const p = duel.activePlayer;
    if (duel.phase === 'main1') await aiMain1(duel, 0);
    else if (duel.phase === 'block') duel.declareBlockers(chooseBlocks(duel));
    else if (duel.phase === 'main2') await aiMain2(duel, 0);
    else if (duel.phase === 'attack') duel.declareAttackers(p, []);
    else throw new Error('stuck in phase ' + duel.phase);
  }
  if (duel.winner === null) return { winner: null, turns: duel.turn };
  return { winner: duel.winner, turns: duel.turn };
}
...
for (const a of roster) for (const b of roster) {
  ...
  for (let g = 0; g < games; g++) {
    try {
      const r = g % 2 ? await play(b, a) : await play(a, b);
      const winnerIsA = g % 2 ? r.winner === 1 : r.winner === 0;
      if (r.winner === null) draws++; else if (winnerIsA) wa++; else wb++;
      turns += r.turns;
    } catch (e) { errors++; if (errors < 5) console.error('ERROR', a.id, 'vs', b.id, e.stack); }
  }
...
if (errors) { console.log(`${errors} games threw errors.`); process.exit(1); }
